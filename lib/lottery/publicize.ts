// 付費牆核心：把完整分析包遮罩成「公開版」。
// 高機率 AI 推薦號的「號碼本身」絕不放進公開 JSON (否則前端可從原始碼讀到)，
// 只露出機率/分數 + 問號，吸引付費解鎖。完整版留在 server 端 (未來由認證 API 發)。

import type { AnalysisBundle } from "./analyze";

export interface LockedTeaser {
  rank: number;
  score: number; // 機率/分數，露出
  // 號碼刻意不放
}
...
export interface PublicBundle {
  ...
  // 每日報牌 (免費)：中機率參考號 (排名 pick..pick*2)，號碼露出
  freePicks: { n: number; score: number }[];
  // 高機率 AI 牌 (付費)：只露分數，號碼遮罩
...
export function publicize(full: AnalysisBundle): PublicBundle {
  ...
  // score 已由高到低排序
  const top = full.score.slice(0, pick); // 高機率 → 鎖
  const mid = full.score.slice(pick, pick * 2); // 中機率 → 免費報牌
